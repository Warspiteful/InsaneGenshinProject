...
export class View {
    ...
    fillMap() {
        if (this.model.getFamilyFriendly()) {
    ...
    updateTexts() {
        ...
        this.displays.forEach((value, key) => {
            ...
            value.innerText = title + ": " + this.model.getMax(key);
        });
    }

    renderList() {
        let string = "";
        this.model.getCharactersStored().forEach((name) => string = string.concat(name + " " + this.model.getCharArray(name) + "\n"))
        alert(string)
    }
